...
export const GET = async (req: NextRequest) => {
  try {
    ...
    while (uniqueDelegates.size < UNIQUE_DELEGATES_COUNT && hasMore) {
      let data;
      if(dao==="optimism"){
       data = await op_client.query(DELEGATE_QUERY,{first:FETCH_SIZE,skip:newSkip}).toPromise();
      }else{
        data = await arb_client.query(DELEGATE_QUERY,{first:FETCH_SIZE,skip:newSkip}).toPromise();
      }
