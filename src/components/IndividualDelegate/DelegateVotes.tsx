...
`;
  ...
  const fetchProposalDescriptions = async (first = 100, skip = 0) => {
    if (props.daoDelegates === "optimism") {
      const proposalIdsResult: any = await op_client.query(opQuery(first, skip), { address: props.individualDelegate });
      ...
      setPageData(arb_gqdata.data.votes);
          ...
          {isPageLoading ? (
            ...
            dataToShow.map((proposal: any, index: number) => (
              ...
              >
                ...
                >
                  {/* {props.daoDelegates  === "optimism" ? (proposal.proposalId.support === 1
                    ? "For"
                    : proposal.proposalId.support === 0
                    ? "Against"
                    : "Other"): props.daoDelegates === "arbitrum" ? () : ""
                  } */}
                   <NextUITooltip
